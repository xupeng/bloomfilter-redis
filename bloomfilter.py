...
if __name__ == '__main__':
    ...
    if 'benchmark' in sys.argv:
    ...
    elif 'test_timed' in sys.argv:
        f = TimeSeriesBloomFilter(connection=connection, bitvector_key='test_timed_bloomfilter', 
                n=1024*8, k=4, time_resolution=timedelta(microseconds=1000), 
                time_limit=timedelta(microseconds=10000))
        
        assert 'balla' not in f
        f.add('balla')
        assert 'balla' in f
        start = datetime.now()
        ...
        # make sure that the filter doesn't say that balla is in the filter for too long
        while 'balla' in f:
        ...
        f = BloomFilter(connection=connection, bitvector_key='test_bloomfilter', n=1024, k=4)
        f.add('three')
        f.add('four')
        f.add('five')
        f.add('six')
